(ns floyd-warshall-algorithm.core-spec
  (:require [speclj.core :refer :all]
            [floyd-warshall-algorithm.core :as sut]))
...
(def get-vertices #'sut/get-vertices)
(def adjacency-matrix #'sut/adjacency-matrix)
(def parent-matrix #'sut/parent-matrix)
(def build-path #'sut/build-path)
(def build-shortest-path #'sut/build-shortest-path)
...
(describe "floyd-warshall"
  (let [{mini-graph-distances :distances mini-graph-parents :parents} (sut/floyd-warshall mini-graph)
        {graph-distances :distances graph-parents :parents} (sut/floyd-warshall graph)]
...
(describe "floyd-warshall-graph"
  (it "gets all paths for mini graph"
    (let [result {1
                  ...
                  4 {}}]
      (should= result
               (sut/floyd-warshall-graph mini-graph))))
  ...
  (it "gets all paths for graph"
    (let [result {1
                  ...
                  {1 {#{5 4} 6, #{4 1} 2}
                   ...
                   4 {#{5 4} 6}}}]
      (should= result
               (sut/floyd-warshall-graph graph)))))
